...
class Frame(nn.Module):  # FEM module
    ...
    def forward(self, x):
        ...
        Input_Feature = torch.zeros(x.shape[0], self.FrameNum, self.PatchSize*2, device=x.device)
...
class FEA_T(nn.Module):
    def __init__(self,
                 patch_size=16,  # Frame length L (default value: 32)
                 ...
                 overlap=0.2,  # Sliding step length R (default value: 0.5).
    ...
    def forward(self, x, state=None):
        ...
        # preprossess
        B = x.shape[0]
        dlY = self.Embedding(x)  # dlY: (B, 1024, 64)
        # cls_token + pos_Emb
...
def fea_t_128(num_classes:int, sig_size:int):
    model = FEA_T(
        patch_size=16,
        seq_length=sig_size,
        overlap=0.2,
...
def fea_t_1024(num_classes:int, sig_size:int):
    model = FEA_T(
        patch_size=32,
        seq_length=sig_size,
        overlap=0.5,
